	...
	if (is_string($_GET["pagina"])) {
		...
		//Si la string es numérica, define la variable 'pagina'
		 if (is_numeric($_GET["pagina"])) {
			 ...
			 if ($_GET["pagina"] == 1) {
				 header("Location: PropuestasBusc.php?search=".$_GET["search"]."");
				 die();
...
$obtener_todo_BD = "SELECT * FROM propuestas WHERE Titulo_Propuesta LIKE '%$search%' 
OR Cuerpo_Propuesta LIKE '%$search%' OR Propietario LIKE '%$search%'";
